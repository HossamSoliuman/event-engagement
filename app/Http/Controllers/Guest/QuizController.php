...
class QuizController extends Controller
{
    public function status(Request $request, string $slug): JsonResponse
    {
        ...
        $selected = $sessionToken !== ''
            ? $round->questionsForSession($sessionToken)
            : $round->questions()->get()->take($round->questions_per_round ?: 3);
    ...
    public function answer(Request $request, string $slug): JsonResponse
    {
        ...
        $assigned = $round->questionsForSession($data['session_token']);
        $question = $assigned->firstWhere('id', $data['quiz_question_id']);

        if (! $question) {
            return response()->json(['error' => 'This question is not part of your quiz.'], 422);
        }
    ...
    public function results(string $slug): JsonResponse
    {
        ...
        return response()->json([
            ...
            'winner_text' => $event->quiz_winner_text,
            'end_sponsor_logo_url' => $event->quiz_end_sponsor_logo_url,
        ]);
    }
}
