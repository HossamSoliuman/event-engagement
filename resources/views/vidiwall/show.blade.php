...
<body>
    ...
    <script>
        ...
        function startSlideshow(fotos, interval) {
            ...
            function nextSlide() {
                clearTimeout(slideTimer); // always cancel any pending timer first

                // ── guard: each slide can only advance once ──────────────────────────
                let advanced = false;
                const goNext = () => {
                    if (advanced) return; // block the second caller
                    advanced = true;
                    clearTimeout(slideTimer);
                    liveVideo.onended = null; // detach so the old handler never fires late
                ...
                if (f.media_type === 'video' && f.video_url) {
                    // Reset progress bar (no timed animation for videos)
                    progress.style.transition = 'none';
                    progress.style.width = '0%';

                    showVideo(f.video_url, f.uploader, slideIndex + 1, slideFotos.length, goNext);

                    // Fallback: if the video never fires 'ended' (network hiccup, bad codec)
                    // wait a generous 60 s then advance anyway
                    slideTimer = setTimeout(goNext, 60_000);

                } else {
                    // Stop any currently-playing video before showing a photo
                    ...
                    // Animate progress bar for the full interval duration
                    progress.style.transition = 'none';
                    progress.style.width = '0%';
