    ...
    <style>
        ...
        *::after {
        ...
        .search-bar::before {
            ...
            width: 14px;
            height: 14px;
            background: url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="%237878A0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>') center/contain no-repeat;
            pointer-events: none
        }
        ...
        .empty-state .empty-icon .lucide-icon {
            width: 44px;
            height: 44px
        }
        ...
        /* ── Winner card ── */
        .winner-card {
            background: linear-gradient(135deg, rgba(255, 215, 0, .15), rgba(255, 61, 0, .08));
        ...
        .winner-card .trophy .lucide-icon {
            width: 52px;
            height: 52px
        }
...
<body>
        ...
        <nav class="sb-scroll">
            ...
            @if ($activeEvent)
                ...
                <a href="{{ route('admin.fotos.index', $activeEvent) }}"
                    ...
                    @if ($pend > 0)
                        <span class="nav-badge">{{ $pend }}</span>
                    @endif
                </a>
                <a href="{{ route('admin.lottery.index', $activeEvent) }}"
                    class="nav-item {{ request()->routeIs('admin.lottery*') ? 'active' : '' }}">
                    <i class="ni-icon"><i data-lucide="ticket" class="lucide-icon"></i></i> Lottery
                </a>
                <a href="{{ route('admin.voting.index', $activeEvent) }}"
                    class="nav-item {{ request()->routeIs('admin.voting*') ? 'active' : '' }}">
                    <i class="ni-icon"><i data-lucide="trophy" class="lucide-icon"></i></i> Voting
                </a>
                <a href="{{ route('admin.membership.index', $activeEvent) }}"
                ...
                <a href="{{ route('vidiwall.show', $activeEvent->slug) }}" target="_blank" class="nav-item">
                    <i class="ni-icon"><i data-lucide="monitor-play" class="lucide-icon"></i></i> Vidiwall <i
                        data-lucide="arrow-up-right" class="lucide-icon"></i>
                </a>
                <a href="{{ route('event.landing', $activeEvent->slug) }}" target="_blank" class="nav-item">
                    <i class="ni-icon"><i data-lucide="smartphone" class="lucide-icon"></i></i> Guest Page <i
                        data-lucide="arrow-up-right" class="lucide-icon"></i>
                </a>
            @endif
            ...
            @if (auth()->user()->isSuperAdmin())
                ...
                @if (auth()->user()->isAdmin())
                    <a href="{{ route('admin.media-downloads.index') }}"
                        class="nav-item {{ request()->routeIs('admin.media-downloads.*') ? 'active' : '' }}">
                        <i class="ni-icon"><i data-lucide="archive" class="lucide-icon"></i></i> Media Downloads
                    </a>
                @endif
            @endif
    ...
    <div class="main-wrap">
        <header class="topbar">
            <button class="tb-hamburger" onclick="openSidebar()"><i data-lucide="menu" class="lucide-icon"
                    style="width:22px;height:22px"></i></button>
        ...
        <main class="main-content">
            @if (session('success'))
                <div class="alert alert-success"><i data-lucide="check" class="lucide-icon"></i>
                    {{ session('success') }}</div>
            @endif
            @if (session('error'))
                <div class="alert alert-error"><i data-lucide="x" class="lucide-icon"></i> {{ session('error') }}
                </div>
            @endif
            @if ($errors->any())
