...
<div class="stats-grid" style="grid-template-columns:repeat(auto-fit,minmax(160px,1fr));margin-bottom:20px">
    ...
    <div class="stat-card">
        ...
        <div class="stat-value" style="font-size:18px;padding-top:6px">
            @if($event->lottery_drawn) 🎉 Winner Drawn @else 🎰 Open @endif
        </div>
    </div>
</div>
...
<div class="winner-card mb-3" id="winnerCard">
    ...
    @if($winner->email)<p class="text-muted" style="font-size:13px">{{ $winner->email }}</p>@endif
    <p class="text-muted" style="margin-top:8px;font-size:12px">Drawn {{ $winner->won_at?->format('M d Y · H:i') }}</p>
    <div style="margin-top:18px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap">
        <form method="POST" action="{{ route('admin.lottery.reset',$event) }}">
            @csrf <button class="btn btn-outline btn-sm" onclick="return confirm('Reset the lottery? The winner will be cleared.')">↺ Reset Lottery</button>
        </form>
    </div>
</div>
...
<div class="card mb-3" style="border-color:rgba(255,215,0,.25)">
    <div class="card-body" style="text-align:center;padding:40px 20px">
        ...
        <p class="text-muted" style="margin-bottom:24px">{{ $totalCount }} entries in the draw. One lucky winner will be selected at random.</p>
    ...
    <div class="card-header">
        ...
        <form method="GET" action="{{ route('admin.lottery.index',$event) }}" style="display:flex;gap:8px">
            <div class="search-bar">
                <input name="search" class="form-control" placeholder="Search name or phone…" value="{{ $search }}" style="width:220px">
            </div>
            <button class="btn btn-secondary btn-sm">Search</button>
            @if($search) <a href="{{ route('admin.lottery.index',$event) }}" class="btn btn-ghost btn-sm">✕</a> @endif
        </form>
    </div>
    <div class="table-wrap">
    <table class="table">
        <thead>
            <tr><th>#</th><th>Name</th><th>Phone</th><th>Email</th><th>Entered</th><th>Status</th><th></th></tr>
        </thead>
        <tbody>
        ...
        @empty
        <tr><td colspan="7"><div class="empty-state" style="padding:30px">No entries found.</div></td></tr>
        @endforelse
        </tbody>
    </table>
    </div>
...
function animateDraw() {
    ...
    const emojis = ['🎰','🎲','🎟','🎉','✨','🏆','⭐','🎊'];
    let i = 0;
    const spin = setInterval(() => { icon.textContent = emojis[i++ % emojis.length]; }, 120);
    setTimeout(() => {
        clearInterval(spin);
        icon.textContent = '🎊';
