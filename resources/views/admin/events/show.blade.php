...
@section('content')
    ...
    <div class="stats-grid" style="grid-template-columns:repeat(auto-fit,minmax(140px,1fr))">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value c-blue">{{ $event->votes_count }}</div>
        </div>
        <div class="stat-card">
        ...
        <div class="card">
            ...
            <div class="card-body" style="display:flex;gap:20px;align-items:flex-start;flex-wrap:wrap">
                ...
                <div style="flex:1;min-width:160px">
                    ...
                    <div
                        style="background:var(--dark);border:1px solid var(--border);border-radius:8px;padding:8px 12px;font-size:12px;font-family:monospace;margin-bottom:12px;word-break:break-all">
                        {{ $event->getGuestUrl() }}</div>
                ...
                @foreach ($tallies as $i => $t)
                    ...
                    <div style="margin-bottom:12px">
                        ...
                        <div class="progress-bar">
                            <div class="progress-fill {{ $i === 0 ? '' : 'green' }}" style="width:{{ $pct }}%">
                            </div>
                        </div>
                    </div>
                @endforeach
    ...
    @if ($onScreen)
        <div class="card mb-3" style="border-color:var(--gold);background:rgba(255,215,0,.04)">
            ...
            <div class="card-body" style="display:flex;align-items:center;gap:16px">
                ...
                @else
                    <img src="{{ $onScreen->thumbnail_url }}"
                ...
                <div style="flex:1">
                    ...
                    @if ($onScreen->isVideo() && $onScreen->video_duration)
                        <div class="text-muted text-sm">
                            Duration: {{ gmdate('i:s', (int) $onScreen->video_duration) }}
                        </div>
                    @endif
                    <div class="text-muted text-sm">
                        On screen since {{ $onScreen->displayed_at?->diffForHumans() }}
                    </div>
                </div>
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px">
        <a href="{{ route('admin.fotos.index', $event) }}" class="btn btn-primary">
            Foto Queue @if ($event->pending_count > 0)
                <span
                    style="background:rgba(255,255,255,.2);padding:1px 8px;border-radius:10px;font-size:11px">{{ $event->pending_count }}</span>
            @endif
        </a>
        ...
        <div class="card mb-3" style="border-color:rgba(99,102,241,.25);background:rgba(99,102,241,.04)">
            ...
            <div style="padding:8px 16px;display:flex;flex-wrap:wrap;gap:10px">
                @foreach ($moderators as $mod)
                    <div
                        style="display:flex;align-items:center;gap:8px;background:var(--dark);border:1px solid var(--border);border-radius:8px;padding:8px 12px">
                        <img src="{{ $mod->avatar_url }}"
                ...
                @foreach ($recentLog as $log)
                    ...
                    <div class="activity-item">
                        ...
                        <div style="flex:1;font-size:12px">
                            ...
                            @if ($log->meta && isset($log->meta['uploader']))
                                — {{ $log->meta['uploader'] }}
                            @endif
                        </div>
                        <div class="text-xs text-muted">{{ $log->user?->name ?? 'Guest' }} ·
