    ...
    <div class="card-header">
        ...
        <form method="GET" action="{{ route('admin.membership.index',$event) }}" style="display:flex;gap:8px">
            <div class="search-bar">
                <input name="search" class="form-control" placeholder="Name or email…" value="{{ $search }}" style="width:220px">
            </div>
            <button class="btn btn-secondary btn-sm">Search</button>
            @if($search)<a href="{{ route('admin.membership.index',$event) }}" class="btn btn-ghost btn-sm">✕</a>@endif
        </form>
    </div>
    <div class="table-wrap">
    <table class="table">
        <thead>
            <tr><th>Name</th><th>Email</th><th>Phone</th><th>Team</th><th>Newsletter</th><th>Member #</th><th>Joined</th><th></th></tr>
        </thead>
        <tbody>
        ...
        @empty
        <tr><td colspan="8"><div class="empty-state" style="padding:30px">No members found.</div></td></tr>
        @endforelse
        </tbody>
    </table>
    </div>
